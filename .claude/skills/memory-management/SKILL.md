...
> **Local adaptation (Jarvis):** In this repo, `CLAUDE.md` is the identity document
> (soul / role / user / routing) and its four sections are never modified by this skill.
> Working-memory entries live in a clearly marked `## Workplace Memory` section appended
> at the end of `CLAUDE.md`; deep storage lives in `memory/` as described below.
